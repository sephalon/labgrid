...
class ClientSession(ApplicationSession):
    ...
    def copy_files(self):
        ...
        target.activate(drv)
        try:
            drv.copy_files(self.args.filenames,
                           self.args.target_directory, self.args.partition)
        except subprocess.CalledProcessError as e:
            raise UserError(
                "could not copy files to network usb storage: {}".format(e))
        except FileNotFoundError as e:
            raise UserError(e)
...
def main():
    ...
    subparser = subparsers.add_parser('copy-files', help="copy files onto mass storage")
    subparser.add_argument('-n', '--partition', default=1, choices=range(1, 256), metavar='1-255', help='partition number to copy to')
    subparser.add_argument('-t', '--target-directory', type=pathlib.PurePath, default=pathlib.PurePath('.'), metavar='DIRECTORY', help='copy all files into DIRECTORY')
    subparser.add_argument('-w', '--wait', type=float, default=10.0, help='storage poll timeout in seconds')
    subparser.add_argument('filenames', type=pathlib.PurePath, nargs='+', metavar='filename', help='filename to copy onto mass storage')
